...
void rc_gui_create_equalizer()
{
    ...
    for(i=0;i<10;i++)
    {
        g_signal_connect(G_OBJECT(eq_scales[i]), "value-changed",
            G_CALLBACK(rc_gui_set_equalizer),NULL);
...
void rc_gui_set_equalizer(GtkAdjustment *adjustment, gpointer data)
{
    gint i = 0;
    gdouble value[10];
    for(i=0;i<10;i++)
...
void rc_gui_equalizer_load_setting(GtkButton *button, gpointer data)
{
...
void rc_gui_equalizer_save_setting(GtkButton *button, gpointer data)
{
    GtkWidget *file_chooser;
    GtkFileFilter *file_filter1;
    int i = 0;
