...
void rc_gui_lrc_init()
{
    ...
    if(GTK_IS_SOURCE_LANGUAGE(lrc_source_lang))
    {
        ...
        rc_debug_print("GUI: Found lyric language file!\n");
    }
    else
    {
        rc_glrc.lrc_buffer = gtk_source_buffer_new(NULL);
        rc_debug_perror("GUI-ERROR: Cannot found lyric language file!\n");
    }
...
void rc_gui_lrc_enable()
{
    ...
    rc_glrc.lyric_text = rc_lrc_get_text_data();
...
void rc_gui_edit_lyric(GtkMenuItem *widget, gpointer data)
{
        ...
        if(gtk_text_buffer_get_modified(GTK_TEXT_BUFFER(rc_glrc.lrc_buffer)))
        {
            ...
            switch(result)
            {
                case GTK_RESPONSE_YES:
                    rc_gui_lrc_edit_save(NULL, NULL);
                    break;
...
void rc_gui_lrc_edit_replace_tag(GtkWidget *widget, gpointer data)
{
    rc_gui_lrc_edit_delete_tag(widget, data);
    rc_gui_lrc_edit_add_tag(widget, data);
}

void rc_gui_lrc_edit_delete_tag(GtkWidget *widget, gpointer data)
{
    if(rc_glrc.lrc_buffer==NULL) return;
    gint cursor_position = 0;
    gchar *line_text;
...
void rc_gui_lrc_edit_cut_selection(GtkWidget *widget, gpointer data)
{
...
void rc_gui_lrc_edit_copy_selection(GtkWidget *widget, gpointer data)
{
    GtkClipboard *clipboard = gtk_widget_get_clipboard(rc_glrc.lrc_text,
        GDK_SELECTION_CLIPBOARD);
...
void rc_gui_lrc_edit_paste_selection(GtkWidget *widget, gpointer data)
{
    GtkClipboard *clipboard = gtk_widget_get_clipboard(rc_glrc.lrc_text,
        GDK_SELECTION_CLIPBOARD);
...
void rc_gui_lrc_edit_delete_selection(GtkWidget *widget, gpointer data)
{
    gtk_text_buffer_delete_selection(GTK_TEXT_BUFFER(rc_glrc.lrc_buffer),
        TRUE, TRUE);
}

void rc_gui_lrc_edit_mark_set(GtkTextBuffer *textbuffer, GtkTextIter *iter,
...
void rc_gui_lrc_edit_undo(GtkWidget *widget, gpointer data)
{
...
void rc_gui_lrc_edit_redo(GtkWidget *widget, gpointer data)
{
...
void rc_gui_lrc_edit_changed(GtkTextBuffer *textbuffer, gpointer data)
{

}

void rc_gui_lrc_edit_return(GtkWidget *widget, gpointer data)
{
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(
        ui_menu->view_menu_items[4]), FALSE);
}

void rc_gui_lrc_edit_load(GtkWidget *widget, gpointer data)
{
    ...
    switch(result)
    {
        case GTK_RESPONSE_ACCEPT:
            file_name = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(
                file_chooser));
            if(rc_lrc_read_from_file(file_name))
            {
                gtk_source_buffer_begin_not_undoable_action(
                    GTK_SOURCE_BUFFER(rc_glrc.lrc_buffer));
                rc_gui_lrc_enable();
...
void rc_gui_lrc_edit_save(GtkWidget *widget, gpointer data)
{
    GtkWidget *file_chooser;
    GtkFileFilter *file_filter1;
    gint result = 0;
    ...
    switch(result)
    {
        case GTK_RESPONSE_ACCEPT:
            ...
            lrc_new_text = gtk_text_buffer_get_text(
                ...
                TRUE);
            rc_lrc_set_text(lrc_new_text);
            rc_lrc_save_lrc(file_name);
            rc_gui_lrc_enable();
            gtk_text_buffer_set_modified(GTK_TEXT_BUFFER(rc_glrc.lrc_buffer),
                FALSE);
            g_free(lrc_new_text);
