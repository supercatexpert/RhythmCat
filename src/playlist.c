    ...
    gint music_index)
{
    ...
    if(g_regex_match_simple("(.CUE)$", uri_d, G_REGEX_CASELESS, 0))
    {
        ...
        cue_mmd_list = plist_read_cue_file(filename);
        g_free(filename);
    ...
    {
        /* Embeded CUE Sheet */
        cue_mmd_list = plist_read_emb_cue_sheet(&mmd);
...
gboolean plist_play_by_index(gint list_index, gint music_index)
{
    ...
    gint time_min, time_sec;
    gint trackno = -1;
    gint lyric_priv = 0, cover_priv = 0;
    gchar *list_uri = NULL;
    gchar music_linfo[1024];
    gchar list_title[512];
    gchar list_length[64];
    gchar *music_path = NULL;
    gchar *music_dir = NULL;
    gchar *lyric_dir = NULL, *image_dir = NULL;
    ...
    gint i = 0;
    MusicMetaData mmd_new, *cue_mmd;
    GDir *music_gdir, *lrc_gdir, *img_gdir;
    gchar *music_pfname = NULL;
    const gchar *filename_foreach = NULL;
    ...
    if(!gtk_tree_model_get_iter(GTK_TREE_MODEL(list_store), &iter, path))
    {
        ...
        if(path_old!=NULL) gtk_tree_path_free(path_old);
        return FALSE;
    }
    ...
    {
        rc_debug_print("Trying to play CUE file...\n");
        fpathname = g_filename_from_uri(list_uri, NULL, NULL);
        cue_mmd_list = plist_read_cue_file(fpathname);
        g_free(fpathname);
        fpathname = NULL;
        if(cue_mmd_list==NULL)
        {
            rc_debug_print("ERROR: Cannot open CUE file!\n");
            if(path_old!=NULL) gtk_tree_path_free(path_old);
            return FALSE;
        }
        for(cue_list_foreach=cue_mmd_list;cue_list_foreach!=NULL;
        ...
        if(!mmd_new.cue_flag)
        {
            rc_debug_print("ERROR: Cannot find required track!\n");
            if(path_old!=NULL) gtk_tree_path_free(path_old);
            return FALSE;
        }
    }
    else
    {
        ...
        {
            rc_debug_print("Trying to play embeded CUE Sheet\n");
            cue_mmd_list = plist_read_emb_cue_sheet(&mmd_new);
            if(cue_mmd_list==NULL)
            {
                rc_debug_print("ERROR: Cannot open embeded CUE Sheet! Play "
    ...
    core_set_uri(mmd_new.uri);
    if(mmd_new.cue_flag)
    {
        gcore->cue_start_time = mmd_new.cue_start_time * 10000000;
        gcore->cue_end_time = mmd_new.cue_end_time * 10000000;
    }
    else
    {
        gcore->cue_start_time = -1;
        gcore->cue_end_time = -1;
    }
    rc_debug_print("Play music file: %s\n", list_uri);
    gui_set_music_info_label(list_title, mmd_new.artist, mmd_new.album, 
        mmd_new.file_type, bitrate);
    ...
    g_free(list_uri);
    if(music_path==NULL) return flag;
    ...
    if(music_basename!=NULL)
    {
        dot_pointer = strrchr(music_basename, '.');
        if(dot_pointer!=NULL)
        {
            music_pfname = g_strndup(music_basename, 
                (gsize)(dot_pointer - music_basename));
        }
        else music_pfname = g_strdup(music_path);
        g_free(music_basename);
    }
    bzero(music_linfo, 1024);
    g_snprintf(music_linfo, 1024, "%s - %s ", music_artist, list_title);
    music_gdir = g_dir_open(music_dir, 0, NULL);
    lrc_gdir = g_dir_open(lyric_dir, 0, NULL);
    img_gdir = g_dir_open(image_dir, 0, NULL);
    while((filename_foreach=g_dir_read_name(music_gdir))!=NULL)
    {
        if(lyric_priv<5 && g_regex_match_simple("(.LRC)$",
            filename_foreach, G_REGEX_CASELESS, 0))
        {
            if(music_pfname!=NULL && g_str_has_prefix(filename_foreach,
                music_pfname) && !mmd_new.cue_flag)
            {
                if(lyric_filename!=NULL) g_free(lyric_filename);
                lyric_filename = g_strdup_printf("%s%c%s", music_dir,
                    G_DIR_SEPARATOR, filename_foreach);
                lyric_priv = 5;
            }
            else if(lyric_priv<4 && g_str_has_prefix(filename_foreach,
                music_linfo))
            {
                if(lyric_filename!=NULL) g_free(lyric_filename);
                lyric_filename = g_strdup_printf("%s%c%s", music_dir,
            ...
            else if(lyric_priv<3 && g_str_has_prefix(filename_foreach,
                mmd_new.title))
            {
                if(lyric_filename!=NULL) g_free(lyric_filename);
                lyric_filename = g_strdup_printf("%s%c%s", music_dir,
        ...
        else if(cover_priv < 5 && g_regex_match_simple(
            "(.BMP|.JPG|.JPEG|.PNG)$", filename_foreach, G_REGEX_CASELESS, 0))
        {
            if(g_str_has_prefix(filename_foreach, mmd_new.album))
            {
                if(cover_filename!=NULL) g_free(cover_filename);
                cover_filename = g_strdup_printf("%s%c%s", music_dir,
                    G_DIR_SEPARATOR, filename_foreach);
                cover_priv = 5;
            }
            else if(cover_priv<4 && g_str_has_prefix(filename_foreach,
                music_pfname))
            {
                if(cover_filename!=NULL) g_free(cover_filename);
                cover_filename = g_strdup_printf("%s%c%s", music_dir,
            ...
            else if(lyric_priv<3 && mmd_new.artist[0]!='\0' && 
                g_str_has_prefix(filename_foreach, mmd_new.artist))
            {
                if(cover_filename!=NULL) g_free(cover_filename);
                cover_filename = g_strdup_printf("%s%c%s", music_dir,
                    G_DIR_SEPARATOR, filename_foreach);
    ...
    g_dir_close(music_gdir);
    if(lyric_filename==NULL && lrc_gdir!=NULL)
    {
        lyric_priv = 0;
        while((filename_foreach=g_dir_read_name(lrc_gdir))!=NULL)
        {
            if(lyric_priv<5 && g_regex_match_simple("(.LRC)$",
                filename_foreach, G_REGEX_CASELESS, 0))
            {
                ...
                {
                    if(lyric_filename!=NULL) g_free(lyric_filename);
                    lyric_filename = g_strdup_printf("%s%c%s", lyric_dir,
                        G_DIR_SEPARATOR, filename_foreach);
                    lyric_priv = 5;
                    break;
                }
    ...
    if(lyric_dir!=NULL) g_free(lyric_dir);
    if(lrc_gdir!=NULL) g_dir_close(lrc_gdir);
    if(cover_filename==NULL)
    {
        cover_priv = 0;
        while((filename_foreach=g_dir_read_name(img_gdir))!=NULL)
        {
            if(cover_priv<5 && g_regex_match_simple("(.BMP|.JPG|.JPEG|.PNG)$",
                filename_foreach, G_REGEX_CASELESS, 0))
            {
                if(music_pfname!=NULL && g_str_has_prefix(filename_foreach,
                    mmd_new.album))
                {
                    if(cover_filename!=NULL) g_free(cover_filename);
                    cover_filename = g_strdup_printf("%s%c%s", image_dir,
                        G_DIR_SEPARATOR, filename_foreach);
                    cover_priv = 5;
                    break;
                }
    ...
    if(img_gdir!=NULL) g_dir_close(img_gdir);
    if(music_pfname!=NULL) g_free(music_pfname);
    if(lyric_filename!=NULL && lrc_open_lyric_from_file(lyric_filename))
    {
        rc_debug_print("Found lyric file: %s, enable the lyric show.\n",
            lyric_filename);
        gui_lrc_enable();
    }
    else
    ...
    if(lyric_filename!=NULL) g_free(lyric_filename);
    if(cover_filename!=NULL && gui_set_cover_image(cover_filename))
    {
        rc_debug_print("Found cover image file: %s.\n",
            cover_filename);
    }
    else gui_set_cover_image(NULL);
    return flag;
}
...
GSList *plist_read_cue_file(gchar *filename)
{
    ...
    bzero(&music_md, sizeof(MusicMetaData));
    uri = g_filename_to_uri(music_filename, NULL, NULL);
    plist_load_metadata(uri, &music_md, &errorno);
    g_free(uri);
    if(errorno!=0)
    {
    ...
    for(i=0;line_data_array[i]!=NULL;i++)
    {
            ...
            line_data, G_REGEX_CASELESS, 0))
        {
            ...
            rc_debug_print("Got track %d from CUE File: %s\n", track_num,
                filename);
        }
...
GSList *plist_read_emb_cue_sheet(MusicMetaData *cue_mmd)
{
